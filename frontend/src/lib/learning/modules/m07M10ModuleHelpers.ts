...
const STORYBOARD = "docs/storyboards/VERBATIM_M07_M10.md";
...
export function sources(sceneId: string, requirementIds: string[]): SourceReference[] {
  return [
    ...
    },
    {
      document: STORYBOARD,
      section: sceneId,
      requirementIds: [`VERBATIM-${sceneId}`],
    },
  ];
}
...
export function responsiveLayout(
  delta: string,
  viewerPlacement: SceneLayoutContract["viewerPlacement"] = "inside_task",
): SceneLayoutContract {
  return {
    desktop: `72 px header; 216 px module rail; data-native ECG/model stage in columns 3–9; 340–380 px Luna panel in columns 10–12; tracing width at least 680 px. ${delta}`,
    laptop: `60 px header and one-line scene strip; seven-column stage and five-column Luna panel; Focus tracing may collapse Luna behind Reopen Luna; tracing width at least 560 px. ${delta}`,
    mobile: `Scene title → essential copy → ECG/model → manipulation → feedback → Ask Luna sheet → navigation. Use a pannable lead board with a persistent lead mini-map; never shrink below legible small-box spacing. ${delta}`,
    focusOrder: [
      "scene heading",
      "essential lesson copy",
      "ECG or teaching model",
...
export function tutorContract(values: {
  ...
  return {
    ...
    tangentBridge:
      values.tangentBridge ??
      "Let’s take that tangent. I’ll keep your exact place in this scene.",
    returnPrompt: values.returnPrompt,
    caseUnavailablePrompt:
      "This case cannot support the promised task. It has been removed from your session and will not affect mastery.",
  };
}
...
export function accessibility(values: {
  ...
  return {
    ...
    reducedMotionAlternative:
      ...
      "Use numbered static frames with Previous step and Next step; no information depends on motion or color.",
  };
}
  ...
  return [
    {
      id: "correct",
      when: "correct",
      heading: "Evidence aligned",
      body: values.correct,
    },
    {
      id: "partial",
      when: "partially_correct",
      heading: "Partly supported",
      body: values.partial,
      evidenceCue: values.evidenceCue,
    },
    {
      id: "incorrect",
      when: "incorrect",
      heading: "Not yet",
      body: values.incorrect,
      evidenceCue: values.evidenceCue,
    },
    {
      id: "unsafe",
      when: "unsafe",
      heading: "Pause at the tracing",
      body: values.unsafe,
      evidenceCue: values.evidenceCue,
    },
    {
      id: "not-assessable",
      when: "not_assessable",
      heading: "Assessability check",
      body: values.notAssessable,
      evidenceCue: values.evidenceCue,
    },
  ];
}
...
): ProductionScene["completionRule"] {
  return {
    requiredInteractionIds: interactions
...
export function buildScene(
  values: Omit<ProductionScene, "completionRule"> & {
    minimumScore?: number;
    retry?: boolean;
  },
): ProductionScene {
  ...
  values: CompactInteractionBase &
    (
      ...
      | {
          ...
          target: Extract<LearningInteraction, { kind: "caliper" }>["target"];
      ...
      | {
          ...
          toleranceDeg: number;
          targetLabel: string;
          predictions?: Extract<LearningInteraction, { kind: "vector_lab" }>["predictions"];
        }
      | {
          kind: "compare";
      ...
      | {
          kind: "sequence";
          cards: Extract<LearningInteraction, { kind: "sequence" }>["cards"];
          correctOrder: string[];
        }
      | {
          kind: "single_select";
          options: Extract<LearningInteraction, { kind: "single_select" }>["options"];
          correctOptionId: string;
        }
      | {
          kind: "multi_select";
          options: Extract<LearningInteraction, { kind: "multi_select" }>["options"];
      ...
      | {
          ...
          rubric: Extract<LearningInteraction, { kind: "free_response" }>["rubric"];
          sentenceFrame?: string;
        }
      | {
          kind: "clinical_stage";
          stages: Extract<LearningInteraction, { kind: "clinical_stage" }>["stages"];
        }
      | {
          kind: "lead_select";
      ...
      | {
          ...
          selectionMode: "single" | "multiple" | "ordered";
          hotspots: Extract<LearningInteraction, { kind: "hotspot_map" }>["hotspots"];
      ...
      | {
          ...
          model:
            ...
            | "reentry"
            | "repolarization";
          frames: Extract<LearningInteraction, { kind: "model_explore" }>["frames"];
      ...
      | {
          ...
          target: Extract<LearningInteraction, { kind: "numeric_entry" }>["target"];
          minimum?: number;
          maximum?: number;
        }
      | {
          kind: "pairing";
          left: Extract<LearningInteraction, { kind: "pairing" }>["left"];
          right: Extract<LearningInteraction, { kind: "pairing" }>["right"];
          correctPairs: Record<string, string>;
        }
      | {
          kind: "categorize";
          items: Extract<LearningInteraction, { kind: "categorize" }>["items"];
          categories: Extract<LearningInteraction, { kind: "categorize" }>["categories"];
